...
if __name__ == "__main__":
    ...
    config.model_folder = "nir_identification"
    config.seed = 1337
        ...
        for fold in [1,2,3]:
            for q_level in [8, 6, 4]:
                ...
                for step in steps:
                    ...
                    config.results_file = f"{config.results_path}/{model}_id_q{q_level}_nir{flipping}"
                    
                    config.test_model_id = f"{model}_id_f{fold}_w{q_level}a{q_level}_NIR-Images{flipping}"
                    ...
                    config.results_file = f"{config.results_path}/{model}_id_f-all_w{q_level}a{q_level}_nirxspectrum{flipping}.txt"
                    ...
                    with open(f"./xspectrum_identification/config_id_nirxspectrum_test_{r:05d}.pkl", "wb") as f:
