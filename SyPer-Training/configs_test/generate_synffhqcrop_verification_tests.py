...
if __name__ == "__main__":
    ...
    config.model_folder = "synffhqcrop_verification"
    config.seed = 1337
    ...
    """
    types_models = ["resnet50"]#,"resnet18",  "mobilefacenet"]
    ...
    for model in types_models:
        for fold in [1,2,3]:
            for q_level in [8, 6, 4]:
                ...
                config.results_file = f"{config.results_path}/{model}_ver_q{q_level}_synffhqcrop{flipping}"
                
                config.test_model_id = f"{model}_ver_f{fold}_w{q_level}a{q_level}_synffhqcrop{flipping}"
                ...
                config.results_file = f"{config.results_path}/{model}_ver_f-all_w{q_level}a{q_level}_synffhqcrop{flipping}.txt"
                ...
                with open(f"./synffhqcrop_verification/config_ver_synffhqcrop_test_{r:05d}.pkl", "wb") as f:
