...
if __name__ == "__main__":
    ...
    config.model_folder = "synffhqgan_verification"
    config.seed = 1337
    ...
    for model in types_models:
        for fold in [1,2,3]:
            for q_level in [8, 6, 4]:
                ...
                config.results_file = f"{config.results_path}/{model}_ver_q{q_level}_synffhqgan{flipping}"
                
                config.test_model_id = f"{model}_ver_f{fold}_w{q_level}a{q_level}_synffhqgan{flipping}"
                ...
                config.results_file = f"{config.results_path}/{model}_ver_f-all_w{q_level}a{q_level}_synffhqgan_nirxspectrum{flipping}.txt"
                ...
                with open(f"./synffhqgan_verification_nir/config_ver_synffhqgan_nirxspectrum_test_{r:05d}.pkl", "wb") as f:
